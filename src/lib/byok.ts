...
export const BUILTIN_PROVIDERS: LLMProvider[] = [
  ...
  },
  {
    ...
    defaultModel: 'gemini-2.5-flash', authType: 'query-param',
    models: ['gemini-2.5-pro', 'gemini-2.5-flash', 'gemini-2.0-flash'],
    helpUrl: 'https://aistudio.google.com/apikey', color: '#4285f4',
  },
  {
...
export async function loadBYOKConfig(request: Request, env: any): Promise<BYOKConfig | null> {
  // 1. URL params (highest priority — ephemeral)
  ...
  // 4. KV (persistent cloudflare sync)
...
/** Encrypt API key with user-derived key (simple XOR for Workers — real encryption would use Web Crypto AES-GCM) */
...
export async function callLLM(
  ...
  if (config.activeProvider === 'google') {
    return callGoogleFormat(baseUrl, apiKey, model, messages, { stream, maxTokens });
  }
...
function callGoogleFormat(
  ...
  return new Response(body, {
    headers: { 'Content-Type': 'application/json', 'X-LLM-Proxy': 'google' },
    status: 200,
  });
}
