# CLAUDE.md — Playerlog Ai

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Playerlog Ai
- **Role**: AI-powered Playerlog companion
- **URL**: https://playerlog-ai.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/playerlog-ai
- **Branch**: master
...
### Deploy
```bash
cd /tmp/playerlog-ai && wrangler deploy
```

### Health Check
```bash
curl -s https://playerlog-ai.casey-digennaro.workers.dev/health
curl -s https://playerlog-ai.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=playerlog-ai
...
- **vessel.json capabilities**: gaming-logs, coaching-tools
...
1. Keep the vessel small. ~246 lines is the current size.
